...
# YK: remove padding from make_grid
from functools import partial
make_grid = partial(make_grid, padding=0)
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        elif getattr(self, "denoise_type", None) not in ["d"]:
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        cc_input_size = kwargs.pop("cc_input_size", 773)
        ...
        # self.cc_projection = nn.Linear(cc_input_size, 768)
    ...
    def _get_denoise_row_from_list(self, samples, desc='', force_no_decoder_quantization=False):
        denoise_row = []
        samples = samples["pred_x0"] if isinstance(samples, dict) else samples
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        batch = batch[-1] if isinstance(batch, list) else batch
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # print(f"cc gradient: {self.cc_projection[-1].weight.grad}")
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if unconditional_guidance_scale > 1.0:
            # get unconditional guidance 
            cond_bs = c['c_crossattn'][0].shape[0]
            uc = {"c_crossattn": [self.get_learned_conditioning([""]).repeat(cond_bs,1,1)], "c_concat":[]}
            uc["c_crossattn"].extend([_c.clone().detach() for _c in c['c_crossattn'][1:]])
    ...
    def configure_optimizers(self):
        ...
        cross_blocks = getattr(self.model.diffusion_model, 'cross_blocks', [])

        # list of raw modules and parameters
        cross_params = [list(block.parameters()) if isinstance(block, torch.nn.Module) else [block] for block in cross_blocks]
        cross_params = set(sum(cross_params, []))
        model_no_cross_params = model_params - cross_params

        print(f"model_params: {len(model_params)}, cross_params: {len(cross_params)}, model_no_cross_params: {len(model_no_cross_params)}")
        assert len(model_params) == len(cross_params) + len(model_no_cross_params)

        param_groups = [
            {"params": list(model_no_cross_params), "lr": lr}, {"params": self.cc_projection.parameters(), "lr": 10. * lr}
            ]
        
        if len(cross_params) > 0:
            param_groups.append({"params": list(cross_params), "lr": 3. * lr})
            print(f"optimizing cross_params with lr: {3. * lr}")
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        assert self.conditioning_key in [None, 'concat', 'crossattn', 'hybrid', 'adm', 'hybrid-adm', 'hybrid-mv']
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None):
        if self.conditioning_key is None:
            ...
            xc = torch.cat([x] + c_concat, dim=1)
            try:
                cc = torch.cat(c_crossattn, 1) if isinstance(c_crossattn, list) else c_crossattn
            except:
                # separate cams are passed via crossattn
                cc = c_crossattn
            out = self.diffusion_model(xc, t, context=cc)
        
        # multiview diffusion
        elif self.conditioning_key == 'hybrid-mv':
            if len(x.shape) == 5:
